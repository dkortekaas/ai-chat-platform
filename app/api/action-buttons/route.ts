...
export async function GET(request: NextRequest) {
  try {
    ...
    const actionButtons = await prisma.actionButton.findMany({
      where: {
        assistantId
      },
      orderBy: [
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { assistantId, buttonText, question, priority = 50, enabled = true } = body
    ...
    const actionButton = await prisma.actionButton.create({
      data: {
        assistantId,
        buttonText,
        question,
